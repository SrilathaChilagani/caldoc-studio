                ...
                {heldReservations.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className={`${tdCls} space-y-1.5`}>
                      <AssignPatientDialog reservationId={r.id} friendlyId={r.friendlyId} providerName={r.providerName} slotTime={formatSlot(r.slotTime)} />
                      <div className="flex gap-1.5">
                        <EditReservationDialog reservationId={r.id} initialAmountPaise={r.amountPaise} initialNotes={r.notes} />
              ...
              <tbody className="divide-y divide-border">
                ...
                {confirmedAppointments.map((appt) => (
                  <tr key={appt.id}>
                    ...
                    <td className={tdCls}><span className="font-medium text-foreground">{appt.patientName}</span></td>
                    <td className={tdCls}>{appt.providerName}</td>
                    <td className={tdCls}><Badge variant="secondary" className="rounded-full bg-primary/10 text-primary">{appt.speciality}</Badge></td>
                    <td className={`${tdCls} whitespace-nowrap text-xs`}>{formatSlot(appt.slotTime)}</td>
                    <td className={tdCls}>
                      <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${statusClasses[appt.status] || "bg-muted text-muted-foreground"}`}>{appt.status}</span>
                    </td>
                    <td className={`${tdCls} text-right font-semibold`}>{formatCurrency(appt.feePaise)}</td>
                    <td className={`${tdCls} space-y-1`}>
                      {appt.hasPrescription && <Button size="sm" variant="outline" className="rounded-full text-xs text-primary border-primary/30 w-full">Prescription</Button>}
                      {appt.hasReceipt && <Button size="sm" variant="outline" className="rounded-full text-xs w-full">Receipt</Button>}
                    </td>
                  </tr>
                ))}
              </tbody>
