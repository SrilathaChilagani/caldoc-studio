            ...
            <Card className="rounded-3xl">
              <CardContent className="p-6">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    <h1 className="text-2xl font-semibold text-foreground">Labs queue</h1>
                    <p className="text-sm text-muted-foreground">Track lab orders, assign collection agents, and manage results.</p>
                  </div>
                      ...
                      {labOrders.map((order) => (
                        <tr key={order.id} className="border-b border-border/50 last:border-0 align-top">
                          <td className="py-3 pr-4">
                            <div className="text-xs font-medium text-foreground">{formatIST(order.createdAt)}</div>
                            <div className="font-mono text-[10px] text-muted-foreground">{order.id}</div>
                            {order.notes && <p className="mt-0.5 text-[10px] text-muted-foreground italic">{order.notes}</p>}
                          </td>
                          <td className="py-3 pr-4 max-w-[160px]">
                            <ul className="space-y-0.5">{order.tests.map((t, idx) => <li key={idx} className="text-xs text-foreground">• {t.name}{t.qty > 1 ? ` ×${t.qty}` : ""}</li>)}</ul>
                            <p className="mt-1 text-xs font-semibold text-foreground">{formatINR(order.amountPaise)}</p>
                          </td>
                          <td className="py-3 pr-4">
                            <div className="font-medium text-foreground">{order.patientName}</div>
                            <div className="font-mono text-xs text-muted-foreground">{order.patientPhone}</div>
                            {order.patientEmail && <div className="text-[10px] text-muted-foreground">{order.patientEmail}</div>}
                          </td>
                          <td className="py-3 pr-4 max-w-[160px]">
                            <Badge variant="secondary" className="rounded-full text-[11px]">{MODE_LABELS[order.deliveryMode] ?? order.deliveryMode}</Badge>
                            {order.address && <p className="mt-1.5 text-[11px] text-muted-foreground">{order.address}</p>}
                          </td>
                          <td className="py-3 pr-4">
                            {order.collectionAgentName ? (
                              <div>
                                <p className="text-xs font-medium text-indigo-700 dark:text-indigo-400">{order.collectionAgentName}</p>
                                {order.collectionAgentPhone && <p className="font-mono text-xs text-indigo-600 dark:text-indigo-400">{order.collectionAgentPhone}</p>}
                              </div>
                            ) : <span className="text-xs text-muted-foreground">Not assigned</span>}
                          </td>
                          <td className="py-3 pr-4">
                            <div className="font-medium text-foreground">{order.providerName}</div>
                            <div className="text-xs text-muted-foreground">{order.providerSpeciality}</div>
                          </td>
                          <td className="py-3">
                            <div className="space-y-2">
                              <Badge className={`rounded-full text-xs font-semibold uppercase ${STATUS_COLORS[order.status] ?? "bg-muted text-muted-foreground"}`}>{STATUS_LABELS[order.status] ?? order.status}</Badge>
                              <Button size="sm" variant="outline" className="rounded-full text-xs block">Update status</Button>
                              {["PROCESSING", "SAMPLE_COLLECTED"].includes(order.status) && <Button size="sm" variant="outline" className="rounded-full text-xs block text-primary border-primary/30">Upload results</Button>}
                              <Button size="sm" variant="ghost" className="rounded-full text-xs block text-primary">Full timeline →</Button>
                            </div>
                          </td>
                        </tr>
                      ))}
