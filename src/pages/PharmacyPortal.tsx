              ...
              <CardContent className="p-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Track every appointment to pack and dispatch prescriptions.</p>
                  </div>
                      ...
                      {appointmentRows.map((appt) => (
                        <tr key={appt.id} className="border-b border-border/50 last:border-0">
                          <td className="py-3 pr-4 text-xs text-muted-foreground">
                            <div className="font-medium text-foreground">{appt.createdAt}</div>
                            <div className="font-mono text-[10px]">{appt.id.slice(-8)}</div>
                          </td>
                          <td className="py-3 pr-4">
                            <div className="font-semibold text-foreground">{appt.providerName}</div>
                            <div className="text-xs text-muted-foreground">{appt.providerSpeciality}</div>
                          </td>
                          <td className="py-3 pr-4">
                            <div className="font-semibold text-foreground">{appt.contactName}</div>
                            <div className="font-mono text-xs text-muted-foreground">{appt.contactPhone}</div>
                          </td>
                          <td className="py-3 pr-4">
                            <Badge variant="secondary" className="rounded-full">{appt.delivery}</Badge>
                            {appt.address ? <p className="mt-1.5 text-xs text-muted-foreground max-w-[180px]">{appt.address}</p> : <p className="mt-1.5 text-xs text-muted-foreground">No address</p>}
                          </td>
                          <td className="py-3 pr-4">
                            <Badge className="rounded-full bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400">{appt.fulfillmentLabel}</Badge>
                            <div className="mt-2"><Button size="sm" variant="outline" className="rounded-full text-xs">Next step →</Button></div>
                          </td>
                          <td className="py-3">
                            {appt.hasPrescription ? <Button size="sm" variant="outline" className="rounded-full text-xs text-primary border-primary/30">Download Rx</Button> : <span className="text-xs text-muted-foreground">Waiting on doctor</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                    ...
                    <tbody>
                      {rxOrders.map((order) => (
                        <tr key={order.id} className="border-b border-border/50 last:border-0 align-top">
                          ...
                          <td className="py-3">
                            <Badge className={`rounded-full text-xs font-semibold uppercase ${rxStatusClasses[order.status] || "bg-muted text-muted-foreground"}`}>{rxStatusLabels[order.status] || order.status}</Badge>
                            <div className="mt-2"><Button size="sm" variant="outline" className="rounded-full text-xs">Update</Button></div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
